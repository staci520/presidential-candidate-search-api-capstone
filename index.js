...
//format query parameters for the URL
...
//function to call Vote Smart API by candidate name
function getVoteSmart(query, candidateName) {
  ...
  fetch(url)
    ...
    .catch(err => {
      $('#js-votesmart-error-message').text(`Something went wrong: ${err.message}`);
    });
}

//function to display VoteSmart results
...
function displayVoteSmart(responseJson, candidateName) {
  ...
  //loop to search for current and former candidates
...
function getNews(query, maxResults = 2) {
  ...
  fetch(url1, options)
    ...
    .catch(err => {
      $('#js-news-error-message').text(`Something went wrong: ${err.message}`);
    });

  //fetch everything (all news)
  fetch(url2, options)
    .then(response => {
      if (response.ok) {
        return response.json();
...
//function to display News API results
...
//function to get YouTube videos of candidate clicked

function getYouTubeVideos(candidateName) {

  const apiKeyYouTube = 'AIzaSyCO_EH4BcSR9BjjY0PtwiVWIfTLLw6-U5I';
...
//function that creates hover effect on selected image 
